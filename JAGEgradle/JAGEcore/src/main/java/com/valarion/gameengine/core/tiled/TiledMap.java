/**
 * Class modified by Rubén Tomás Gracia to implement objects as shapes and correct
 * a bug on object layers throwing exception because of trying to parse width and height
 * when an object layer doesn't have them.
 */
...
public class TiledMap {
	...
	private void load(InputStream in, String tileSetsLocation) throws SlickException {
		...
		try {
			...
			/*
			 * if (!orient.equals("orthogonal")) { throw new SlickException(
	...
	/**
	 * Returns the shape of a specific object from a specific group.
	 * 
	 * @param groupID
	 *            Index of a group
	 * @param objectID
	 *            Index of an object
	 * @return The shape of an object, or null, when error occurred
	 */
	public Shape getObjectShape(int groupID, int objectID) {
		if (groupID >= 0 && groupID < objectGroups.size()) {
			ObjectGroup grp = (ObjectGroup) objectGroups.get(groupID);
			if (objectID >= 0 && objectID < grp.objects.size()) {
				GroupObject object = (GroupObject) grp.objects.get(objectID);
	...
	protected class GroupObject {
		...
		/** Shape that defines the the object */
		public Shape shape;
			...
			for (int i = 0; i < childs.getLength(); i++) {
				...
				if ((n instanceof Element) && n.getNodeName() != null) {
					...
					if ("ellipse".equals(n.getNodeName())) {
						...
						break;
					} else if ("polygon".equals(n.getNodeName())) {
						...
						for (String point : points) {
							...
							try {
								p.addPoint(x + (int) Float.parseFloat(coordstr[0]),
