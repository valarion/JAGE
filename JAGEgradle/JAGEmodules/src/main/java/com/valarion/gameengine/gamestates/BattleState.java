...
public class BattleState extends SubState {
	...
	public static enum Attack {
		...
		thrust("Thrust", "Deals 50-60 HP. Negates \"feint and side cut\"."), feintandsidecut("Feint and side cut",
				"Deals 60-80 HP. Negates \"shield defense\"."), shielddefense("Shield defense",
						"Negates \"thrust\". Deals 10-30 damage if enemy used it."), heal("Heal", "Heals 55-65 HP.");
	...
	public BattleState(GameState next, Image enemy, int enemyhp, int playerhp) {
		...
		this.fading = new Fading(Color.black, Color.transparent, 1000);
		this.fading.start();
	...
	public void update(GameContainer container, int delta) throws SlickException {
		...
		if (fading != null) {
			...
			case enemyturn:
				...
				enemyattack = Attack.values()[new Random()
						.nextInt(Attack.values().length - (enemyhp == enemymaxhp ? 1 : 0))];
				options.add(new CommonOption(
						"You used " + playerattack.toString() + ". Enemy used " + enemyattack.toString()));
			...
			case calculatenewhealth:
				...
				text.getOptions()[text.getOptions().length - 1] = new CommonOption("Player: ");

				if (playerhp == nextplayerhp && enemyhp == nextenemyhp) {
					fading = new Fading(Color.transparent, Color.black, 1000);
					fading.start();
	...
	public void render(GameContainer container, Graphics g) throws SlickException {
		...
		if (enemy != null) {
			...
			int maxh = container.getHeight() - menu.getWindow().getWindow().getHeight()
					- tooltip.getWindow().getWindow().getHeight()
					- text.getWindow().getContain().getGraphics().getFont().getLineHeight();
			...
			if (!stance.equals(Stance.calculatenewhealth) || nextenemyhp >= enemyhp
					|| System.currentTimeMillis() % 400 < 200) {
				Color c = Color.white;
		...
		int w = text.getWindow().getContain().getGraphics().getFont()
				.getWidth("                                          ");
		...
		g.drawString(" / " + Integer.toString((int) enemymaxhp),
				x + w + 2 + g.getFont().getWidth(Integer.toString((int) enemymaxhp)), y);

		// Player health bar
		x = menu.getWindow().getWindow().getWidth() + menu.getWindow().getModel().getImage("left").getWidth()
				+ text.getWindow().getContain().getWidth() / 20
				+ (int) (((float) (text.getWindow().getModel().getImage("rightArrow").getWidth())) * 1.5f)
				+ text.getWindow().getContain().getGraphics().getFont().getWidth(" Player: ");
		y = (int) (container.getHeight() - text.getWindow().getWindow().getHeight()
				+ text.getWindow().getModel().getImage("top").getHeight()
				+ text.getWindow().getContain().getGraphics().getFont().getLineHeight() * 6.5f);
		...
		g.drawString(" / " + Integer.toString((int) playermaxhp),
				x + w + 2 + g.getFont().getWidth(Integer.toString((int) playermaxhp)), y);
